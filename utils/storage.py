...
def save_statistics(
    experiment_name, line_to_add, filename="summary_statistics.csv", create=False
):
    summary_filename = os.path.join(experiment_name, filename)
    if create:
...
def get_best_validation_model_statistics(
    experiment_name, filename="summary_statistics.csv"
):
    """
...
def create_json_experiment_log(
    experiment_log_dir, args, log_name="experiment_log.json"
):
...
def update_json_experiment_log_dict(
    key, value, experiment_log_dir, log_name="experiment_log.json"
):
    summary_filename = "{}/{}".format(experiment_log_dir, log_name)
...
def change_json_log_experiment_status(
    experiment_status, experiment_log_dir, log_name="experiment_log.json"
):
    timestamp = datetime.datetime.now().timestamp()
...
def update_json_experiment_log_epoch_stats(
    epoch_stats, experiment_log_dir, log_name="experiment_log.json"
):
    summary_filename = "{}/{}".format(experiment_log_dir, log_name)
    with open(summary_filename) as f:
        summary_dict = json.load(fp=f)
